...
namespace marian
{
  namespace bergamot
  {
    ...
    std::vector<Ptr<const Vocab>> Tokenizer::loadVocabularies(Ptr<Options> options)
    {
    ...
    data::SentenceTuple Tokenizer::tokenize(std::string const &snt)
    {
      // TODO(jerin): Bunch of hardcode here, 1, 0, need to get rid off somehow.
      data::SentenceTuple sentence_tuple(1);
    ...
    TextProcessor::TextProcessor(Ptr<Options> options): tokenizer(options), sentence_splitter(options){}
